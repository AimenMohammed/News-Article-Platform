...
1. **External Ingestion**: Apache NiFi continuously polls external news websites or APIs, scrapes the content, and publishes the raw data to the `news-raw` Kafka topic.
2. **Streaming Ingestion**: The Spark Structured Streaming job (`bronze_ingestion.py`) consumes these messages in micro-batches and writes them to the Bronze Iceberg table.
3. **Batch/Backfill**: The `scrape_news.py` (direct HTTP scraping) and `ingest_news_batch.py` (mock data generation) act as additional ingestion sources for testing and historical backfill.
4. **Transformation & Serving**: The Silver and Gold layers clean, enrich, and aggregate the data before serving it via ClickHouse and Grafana.
...
All Gold tables are migrated to ClickHouse into the `news_analytics` database, optimized for fast, real-time dashboard queries.
...
| `gold_top_keywords` | 1,089 | Extracted trending keywords |
...
| Challenge | Solution |
| :--- | :--- |
| **Catalog mismatches with HDFS** | Implemented `make base` to reinitialize both the catalog and storage simultaneously. |
| **Loss of Grafana dashboards** | Implemented provisioning by storing dashboards as JSON files (`grafana/provisioning/dashboards/`). |
| **Dirty data (NULLs/empties)** | Applied strict data quality rules (e.g., filtering empty titles) within the Silver transformation. |
